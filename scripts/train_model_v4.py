...
def train_and_persist_model_v4(
    train_size: int = 10000,
    train_seed: int = 20260905,
    val_seed: int = 555444333,
    scenario_id: str = "S1_HIGH_NATURAL_RECOVERY",
):
    print("=" * 70, flush=True)
    print("TRAINING AND SERIALIZING INCREMENTAL-MODEL-V4 (PHASE 12)", flush=True)
    print("=" * 70, flush=True)
    ...
    val_split = db.build_dataset(count=5000, seed=val_seed, scenario_id=scenario_id)
    ...
    cfg_str = json.dumps({"scenario": scenario_id, "model": "calibrated_t_learner", "method": "isotonic", "C": 0.5, "train_seed": train_seed})
    config_hash = hashlib.sha256(cfg_str.encode()).hexdigest()

    saved_path = mgr.save_model(
        models=trained_models,
        model_version="incremental-model-v4",
    ...
    print(f"\nModel incremental-model-v4 successfully saved to: {saved_path}", flush=True)
    return saved_path

if __name__ == "__main__":
    train_and_persist_model_v4()
